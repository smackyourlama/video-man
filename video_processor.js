...
async function generateTTS(scriptText) {
    const outputPath = path.join(PROCESSED_DIR, `audio-${Date.now()}.mp3`);
    console.log('Generating TTS audio...');
    // google-tts-api handles chunking internally if we use getAllAudioBase64
    // However, it can timeout on very long scripts. We need a fallback or split logic here too eventually.
    // For now, let's wrap it in a try-catch and return a dummy audio file if it fails so the pipeline doesn't crash completely.
    try {
        const results = await googleTTS.getAllAudioBase64(scriptText, {
            lang: 'en',
            slow: false,
            host: 'https://translate.google.com',
            splitPunc: ',.?!',
            timeout: 60000 // Increase timeout for the TTS fetch
        });
...
async function createThumbnail(imagePath, titleText) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Requires ffmpeg with drawtext (which fluent-ffmpeg/static builds usually have)
        // If drawtext fails, we just return original image.
        ffmpeg(imagePath)
            .outputOptions([
                '-vf', `drawtext=text='${titleText.replace(/'/g, "")}':fontcolor=white:fontsize=72:x=(w-text_w)/2:y=(h-text_h)/2:box=1:boxcolor=black@0.5:boxborderw=10`
            ])
            .output(finalThumb)
...
async function uploadYouTubeVideo(videoPath, title, description, thumbnailPath, channelName) {
    ...
    try {
        ...
        const response = await youtube.videos.insert({
            part: 'snippet,status',
            requestBody: {
                snippet: { title: title.substring(0, 100), description: "Generated by VideoMan" },
